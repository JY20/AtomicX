 ...
 * @dev Factory contract for creating Bitcoin atomic swap escrow contracts
 */
contract BTCEscrowFactory {
    // Events
    ...
    /**
     * @dev Creates a source escrow contract (EVM→BTC)
    ...
    function createSrcEscrow(Immutables memory immutables) external payable returns (address) {
        ...
        address escrow = address(new BTCEscrowSrc{value: msg.value}(immutables));
    ...
    /**
     * @dev Creates a destination escrow contract (BTC→EVM)
     * @param immutables The immutable parameters for the escrow
     * @return The address of the created escrow contract
     */
    function createDstEscrow(Immutables memory immutables) external payable returns (address) {
        // Create escrow contract
        address escrow = address(new BTCEscrowDst{value: msg.value}(immutables));
